...
<html lang="pl">
  ...
  <body>
    ...
    <div id="lewy">
      ...
      if (mysqli_select_db($polaczenie, 'leda')) {
      }else {
        echo "Błędne połączenie z bazą danych leda";}

      $zapytanie = "SELECT `NAZWISKO`,`ETAT`,`PLACA_POD`+COALESCE(`PLACA_DOD`,0) as Placa FROM `pracownicy` ORDER BY 1 ";

      if($rezultat = mysqli_query($polaczenie, $zapytanie)){
        while($wiersz = mysqli_fetch_assoc($rezultat)){
          echo <<<WIERSZ
          $wiersz[nazwisko] pracuje na etacie $wiersz[ETAT] i zarabia $wiersz[placa]
WIERSZ;
        }
      }else {
